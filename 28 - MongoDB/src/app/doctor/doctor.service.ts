...
export class DoctorService {
	async create(firstName: string, lastName: string, specialty: DoctorSpecialty, facilityId: string): Promise<IDoctor> {
	...
	async list(facilityId: string): Promise<IDoctor[]> {
		...
		return await Doctor.find({ facility: facilityId }).populate('facility').exec();
	};
};