...
appointments.post(
	"/",
	withErrorHandler<Request<AppointmentsParams, unknown, AppointmentDTO>>(
		async (req, res) => {
...
appointments.put(
	"/:appointmentId",
	withErrorHandler<Request<AppointmentParams, unknown, AppointmentDTO>>(
		async (req, res) => {
			const userId = req.user;
...
appointments.get(
	...
	withErrorHandler<Request<AppointmentParams>>(async (req, res) => {
		...
		const response = await appointmentService.find(
			userId,
			facilityId,
			doctorId,
			appointmentId
