...
export class AppointmentService {
	...
	async find(userId: string, facilityId: string, doctorId: string, appointmentId: string): Promise<IAppointment> {
		...
		const appointment = await Appointment.findOne({ _id: appointmentId, userId, doctor: doctor.id }).exec();
	...
	async update(userId: string, facilityId: string, doctorId: string, appointmentId: string, startAt: string): Promise<IAppointment> {
