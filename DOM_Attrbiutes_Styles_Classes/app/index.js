...
const USER_DATA = {
	firstName() {
		...
		return STDIN.string(
			`Enter your first name: min length ${MIN}, max length ${MAX}`,
			validate
		);
	},
	lastName() {
		...
		return STDIN.string(
			`Enter your last name: min length ${MIN}, max length ${MAX}`,
			validate
		);
	},
	age() {
		...
		const year = STDIN.number(
			`Enter your year of birth: min ${MIN_YEAR}, max ${MAX_YEAR}`,
			validateYear
		);
		...
		const month = STDIN.number(
			`Enter your month of month: min ${MIN_MONTH}, max ${MAX_MONTH}`,
			validateMonth
		);
		...
		const day = STDIN.number(
			`Enter your month of month: min ${MIN_DAY}, max ${MAX_DAY}`,
			validateDay
		);
...
class App {
	...
	render() {
		for (const item of this.list.children) {
			const fields = Array.from(item.children).filter((child) =>
				child.hasAttribute("data-field")
			);
